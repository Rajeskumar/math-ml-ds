...
def compute_breed_proportions(df):
    ...
    for breed in breeds:
        ...
        prob_class = len(df_breed)/len(df_train)
...
def prob_of_X_given_C(X, features, breed, params_dict):
    ...
    for feature_val, feature_name in zip(X, features):
        ...
        match feature_name:
            ...
            case "bark_days": 
                # Get the relevant parameters out of the params_dict dictionary
                n = params_dict[breed]['bark_days']['n']
                p = params_dict[breed]['bark_days']['p']
            ...
            case "ear_head_ratio": 
                # Get the relevant parameters out of the params_dict dictionary
                a = params_dict[breed]['ear_head_ratio']['a']
                b = params_dict[breed]['ear_head_ratio']['b']
...
def naive_bayes_classifier(text, word_freq=word_freq, class_freq=class_freq):
    ...
    likelihood_word_given_spam = class_freq['spam'] * cumulative_product_spam
    
    # Calculate the likelihood of the words appearing in the email given that it is ham
    likelihood_word_given_ham = class_freq['ham'] * cumulative_product_ham
